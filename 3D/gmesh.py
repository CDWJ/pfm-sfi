...
import numpy as np
import taichi as ti
from geom2d import edge_extractor
...
class TrianMesh:
  ...
  def __init__(
    ...
    if dim == 2 and verts.shape[1] == 3:
      verts = np_3to2(verts)
    edge_indices, edge_sides, edge_neib, face_edges = edge_extractor(
        faces)
    assert faces.ndim == 1
    n_edge = edge_indices.shape[0] // 2
    
    def normalize(v):
        minX = min( [vertex[0] for vertex in v] )
        minY = min( [vertex[1] for vertex in v] )
        minZ = min( [vertex[2] for vertex in v] )
        vMin = np.array([minX, minY, minZ])
        maxDim = np.max([np.max(v[:,0]) - np.min(v[:,0]), \
                            np.max(v[:,1]) - np.min(v[:,1]), \
                                np.max(v[:,2]) - np.min(v[:,2])])
        return maxDim, vMin
    
    cm = verts.sum(0) / verts.shape[0]
    offset = np.array([-cm[0], -cm[1], -cm[2]])
    verts = verts + offset
    maxDim, vMin = normalize(verts)
    verts = ((verts - vMin) / maxDim) * scale
