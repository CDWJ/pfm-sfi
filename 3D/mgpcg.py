...
class MGPCG_3(MGPCG):
    ...
    @ti.kernel
    def apply_bc(self, u_x: ti.template(), u_y: ti.template(), u_z: ti.template(), self_time:float):
        ...
        for i, j, k in u_x:
            if i == 0 and self.boundary_types[0,0] == 2:
                # u_x[i,j,k] = 0.2 * ti.max(ti.sin(2 * math.pi * time / T), 0.0) + 0.1
                u_x[i,j,k] = 0.3
            if i == u_dim - 1 and self.boundary_types[0,1] == 2:
                u_x[i,j,k] = 0.3
            #     u_x[i,j,k] = 0.25 * ti.abs(ti.sin(2 * math.pi * time / T))
    ...
    def Poisson(self, u_x, u_y, u_z, time, verbose = False):
        self.time = time
        self.apply_bc(u_x, u_y, u_z, self.time)
