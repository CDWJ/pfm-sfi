...
@ti.func
def sample_grad(v, x, y, z):
    return ti.Vector([sample(v, x + 1, y, z) - sample(v, x - 1, y, z), sample(v, x, y + 1, z) - sample(v, x, y - 1, z), sample(v, x, y, z + 1) - sample(v, x, y, z - 1)])
# @ti.func
# def sample_grad(v, x, y, z):
#     _, use = interp_grad_2(v, (ti.Vector([x, y, z]).cast(float) + 0.5) * dx, BL_x = 0.5, BL_y = 0.5, BL_z = 0.5)
#     return use
    
# @ti.func
# def sample_grad(v, x, y, z):
#     return ti.Vector([-sample(v, x + 2, y, z) + 8 * sample(v, x + 1, y, z) - 8 * sample(v, x - 1, y, z) + sample(v, x - 2, y, z), 
#                       -sample(v, x, y + 2, z) + 8 * sample(v, x, y + 1, z) - 8 * sample(v, x, y - 1, z) + sample(v, x, y - 2, z), 
#                       -sample(v, x, y, z + 2) + 8 * sample(v, x, y, z + 1) - 8 * sample(v, x, y, z - 1) + sample(v, x, y, z - 2)])
...
@ti.func
def interp_grad_2_onetaylor(vf, p, dx, BL_x = 0.5, BL_y = 0.5, BL_z = 0.5):
    u_dim, v_dim, w_dim = vf.shape
    ...
    partial_z = 0.
    interped = 0.
    particle_second_term = ti.Vector([0.0, 0.0, 0.0])
    ...
    for i in range(-1, 3):
        for j in range(-1, 3):
            for k in range(-1, 3):
                ...
                z_p_z_i = l - (iw + k)
                value = sample(vf, iu + i, iv + j, iw + k)
                # value_grad = sample_grad(vf, iu + i, iv + j, iw + k) / (12 * dx)
                # value_grad = sample_grad(vf, iu + i, iv + j, iw + k) / (2 * dx)
                # value_grad = sample_grad(vf, iu + i, iv + j, iw + k)
                # _, value_grad = interp_grad_2(vf, (ti.Vector([iu + i, iv + j, iw + k]).cast(float) + 0.5) * dx, dx, BL_x = 0.5, BL_y = 0.5, BL_z = 0.5)
                value_new = value + (ti.Vector([-x_p_x_i, -y_p_y_i, -z_p_z_i]).cast(float) * dx).dot(value_grad)
                partial_x += value_new * (1./dx * (dN_2(x_p_x_i) * N_2(y_p_y_i) * N_2(z_p_z_i)))
                partial_y += value_new * (1./dx * (N_2(x_p_x_i) * dN_2(y_p_y_i) * N_2(z_p_z_i)))
                partial_z += value_new * (1./dx * (N_2(x_p_x_i) * N_2(y_p_y_i) * dN_2(z_p_z_i)))
                interped += value_new * N_2(x_p_x_i) * N_2(y_p_y_i) * N_2(z_p_z_i)  
                particle_second_term += value_grad * N_2(x_p_x_i) * N_2(y_p_y_i) * N_2(z_p_z_i)
    return interped, ti.Vector([partial_x, partial_y, partial_z]) + particle_second_term
...
@ti.func
def interp_grad_2_one_taylor_imp(vf, p, dx, BL_x=0.5, BL_y=0.5, BL_z=0.5):
    u_dim, v_dim, w_dim = vf.shape
    ...
    for i in range(-1, 3):
        for j in range(-1, 3):
            for k in range(-1, 3):
                ...
                value = sample(vf, iu + i, iv + j, iw + k)
                value_grad = sample_grad(vf, iu + i, iv + j, iw + k) / (2 * dx)
                value_new = value + (ti.Vector([-x_p_x_i, -y_p_y_i, -z_p_z_i]) * dx).dot(value_grad)
                # imp_value = sample(imp, iu + i, iv + j)
                dw_x = 1. / dx * dN_2(x_p_x_i) * N_2(y_p_y_i) * N_2(z_p_z_i)
                dw_y = 1. / dx * N_2(x_p_x_i) * dN_2(y_p_y_i) * N_2(z_p_z_i)
                dw_z = 1. / dx * N_2(x_p_x_i) * N_2(y_p_y_i) * dN_2(z_p_z_i)
                partial_x += value_new * dw_x
                partial_y += value_new * dw_y
                partial_z += value_new * dw_z
                interped += value_new * N_2(x_p_x_i) * N_2(y_p_y_i) * N_2(z_p_z_i)
                new_C += ti.Vector([dw_x, dw_y, dw_z]) * value_new
                # interped_imp += imp_value * N_2(x_p_x_i) * N_2(y_p_y_i)
